...
def drag_calculation(exp):
    ...
    pressure = pressure_term(
        vel[0], vel[1], u_grad, v_grad, (x, y), re_stress, U)
    print(np.max(vel[2, :, :]))
...
if __name__ == '__main__':
    ...
    U = 16
    x, y, vel = load_data("Velocity_16", zeros=True)
    print("vel ", vel[0, 25, 16])
    # conversion to meters
    # x, y = x[7:], y[7:]
    xx, yy = np.meshgrid(x, y)
    ...
    x, y, std = load_data("StD_vel16", zeros=True)
    # x, y = x[6:], y[6:]
    x, y = x / 100, y / 100
    var_xx = std[0, :, :] ** 2
    var_yy = std[1, :, :] ** 2
    ...
    # # print(conv_drag)
    # #
    # x_b, y_b, stress_zz_b = load_data("Re_bottom_04/B00005.dat")
    # x_b, y_b, stress_zz_b = x_b / 10, y_b / 10, stress_zz_b / U ** 2
    # xx_b, yy_b = np.meshgrid(x_b, y_b)
    # countour_data_plot(xx_b, yy_b, stress_zz_b,
    #                    "Re stress_zz bottom", r"$w'^2$", save=True)
    #
    ...
    # print("shape stress bottom {0} \nShape stress top {1}" .format(
    #     np.shape(stress_zz_b), np.shape(stress_zz_t)))
